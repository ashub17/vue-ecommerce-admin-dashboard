...
const safeOrders = computed(() =>
  Array.isArray(orderStore.orders)
    ? orderStore.orders.filter((order) => order && typeof order === 'object')
    : [],
);
...
async function goToPage(nextPage) {
  if (!nextPage) return;
  if (orderStore.meta && (nextPage < 1 || nextPage > orderStore.meta.last_page))
    return;
